...
def calculate_hidden_flow(
    ...
    answer = answer.strip().strip("▁").strip("Ġ")
    if expect is not None and not (
        answer.startswith(expect) or
        expect.startswith(answer)
        ):
        # print(f"{answer} != {expect}")
        return dict(correct_prediction=False)
    try:
        e_range = find_token_range(mt.tokenizer, inp["input_ids"][0], subject)
    except ValueError:
        raise ValueError("Subject couldn't be found in Prompt")
...
def decode_tokens(tokenizer, token_array):
    ...
    return tokenizer.convert_ids_to_tokens(token_array)


def find_token_range(tokenizer, token_array, substring):
    toks = decode_tokens(tokenizer, token_array)
    whole_string = tokenizer.convert_tokens_to_string(toks)# "".join(toks)
