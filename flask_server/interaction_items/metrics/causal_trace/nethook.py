...
class AutoScale(contextlib.AbstractContextManager):
    def __init__(
        self,
        module,
        layers=None,
        scale_to_dtype=torch.float32,
    ):
        """
        Method to replace a forward method with a closure that
        intercepts the call, and scales the dtype to avoid inf and nan
        Does not work if module is called with kwargs
        """
        def hook(module, input, output):
            if not output.isnan().any() and not output.isinf().any():
                return output

            module_type = next(iter(module.parameters())).dtype
            input_types = [i.dtype for i in input]
            output_types = [o.dtype for o in output] if isinstance(output, tuple) else [output.dtype]
            assert all([t == module_type for t in input_types + output_types])
            original_dtype = module_type

            module = module.to(dtype=scale_to_dtype)
            input = tuple(i.to(dtype=scale_to_dtype) for i in input)

            output = module(*input)

            module.to(dtype=original_dtype)

            return tuple(o.to(dtype=scale_to_dtype) for o in output) if isinstance(output, tuple) else output.to(dtype=original_dtype)

        self.scaling_hooks=[get_module(module, layer).register_forward_hook(hook) for layer in layers]

    def __enter__(self):
        return self

    def __exit__(self, type, value, traceback):
        for hook in self.scaling_hooks:
            hook.remove()


def all_layers_matching(module, template):
    assert template.count("{") == template.count("}") == 1
    pattern = "^" + re.escape(template).replace("\{\}",".*") + "$"
    pattern = re.compile(pattern)
    return [n for n, _ in module.named_modules() if pattern.match(n)]